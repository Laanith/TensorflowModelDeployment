...
function preProcess(image) {
  // const values = imageByteArray(image);  const values = image.bitmap.data;
...
async function ModelLoader(PATH) {
  let ModelLoaderPrediction;
   await Jimp.read(PATH).then(async (image) => {
    ...
    X = X.reshape([1, 784]);
     ...
     ModelLoaderPrediction=noneee;
  });
  console.log('ModelLoaderPrediction in ModelLoader', ModelLoaderPrediction);
  return ModelLoaderPrediction
}
