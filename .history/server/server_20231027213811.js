...
async function ModelLoader(PATH) {
  ...
  await Jimp.read(PATH).then((image) => {
    ...
    MakePrediction(X, modelPath).then((d) => { console.log('final prediction :' , prediction = d) });
...
async function Modeler(PATH) {
  let x;
  await ModelLoader(PATH).then((d) => {
    x = d;
  });
  return d;
}
...
app.post('/', (req, res) => {
    upload(req, res, (err) => { 
      ...
      const pred = ModelLoader(PATH).then((d) => {
        console.log(d);
      })
      // console.log('RESPRED : ', pred);
